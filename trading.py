...
# 1. Actualización del título según el ticker seleccionado
...
# Variable global para la última decisión
ultima_decision = None
# Función de toma de decisión con notificación
def tomar_decision(precio_actual, tendencia, media_sp500,decision, ultima_decision):
    ...
    # Verificar si ha habido un cambio en la decisión
    if decision != ultima_decision:
        if decision == 'Vender':
            st.warning(f"NOTIFICACIÓN: Recomendación de VENTA a un precio de {precio_actual}")
        elif decision == 'Comprar':
            st.success(f"NOTIFICACIÓN: Recomendación de COMPRA a un precio de {precio_actual}")
        else:
            st.info(f"NOTIFICACIÓN: Recomendación de ESPERAR a un precio de {precio_actual}")
        # Actualizar la última decisión
...
if ejecutar:
    ...
    # Inicializar la variable decision
    decision = None
    
    # Tomar decisión
    decision_final = tomar_decision(precio_actual, tendencia, media_sp500, decision, ultima_decision)
    
    # Visualizar el resultado
    visualizacion(datos, decision_final, media_sp500)
